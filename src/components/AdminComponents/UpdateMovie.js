...
export default function UpdateMovie({ movieId }) {
  console.log(movieId," ----------------");
    ...
    useEffect(() => {
      ...
      // let's now get the data of a movie
      fetchApi(`${endpoints.public.singleMovie}/${movieId}`, 'GET')
          .then((data) => {
            setTitle(data.movieTitle);
            setKeyword(data.keywords);
            setImage(data.imageUrl);
            setYear(data.year[0].yearId);
            setDuration(data.duration);
            setReleaseDate(data.releaseDate);
            setSynopsis(data.bigDescription);
            setCast(data.casts);
           setSmallDescription(data.smallDescription);
           setDownloadName(data.downloadName);
            setEpisodes(data.links);
            setCategories(data.genres.map((genre) => genre.genreId));
            setLanguages(data.languages.map((language) => language.languagesId));
            setCollections(data.collections.map((collection) => collection.collectionsId));
    ...
    const handleSubmit = async (event) => {
     ...
     // console.log(categories, "  categories",collections, "  collections",languages, "  languages",episodes, "  episodes",title, "  title",keyword, "  keyword",duration, "  duration",releaseDate, "  releaseDate",synopsis, "  synopsis",cast, "  cast",year, "  year",imageFile, "  imageFile");
      ...
      formdata.append('image', imageFile);  // Append the selected image file
       ...
       await fetchApi(`${endpoints.admin.updateMovie}`, "POST", token.current, null, formdata);
    ...
    return (
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Image */}
          <Image src={`${IMAGE_URL}/${image}`} height={200} width={200} alt={title}/>
          {/* Image Upload */}
          <div>
